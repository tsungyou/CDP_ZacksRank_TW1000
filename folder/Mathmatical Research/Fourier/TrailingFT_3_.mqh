...
double CTrailingFT::ProcessFT(int Index)
   {
      ...
      int _min_index=0;
      double _min=fabs(_output[5][_min_index]);
      for(int s=0;s<5;s++)
      {
         if(_min>fabs(_output[5][s]))
         {
            _min_index=s;
            _min=fabs(_output[5][s]);
         }
      }
      
      _ft=(_output[5][_min_index]/fmax(m_symbol.Point(),fabs(_output[5][_min_index])+fabs(_close)))*100.0;
